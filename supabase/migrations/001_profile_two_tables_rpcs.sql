...
BEGIN
  ...
  ELSIF ctype = 'place' THEN
    SELECT to_jsonb(pl0) INTO pl FROM public.place pl0 WHERE pl0.client_uuid = p_client_uuid;
...
BEGIN
  ...
  INSERT INTO public.client (
    ...
    (CASE WHEN p_type_choice = 'none' OR p_type_choice = '' THEN 'client' ELSE p_type_choice END)::public.client_type,
  ...
  ELSIF p_type_choice = 'place' AND (p_place IS NOT NULL OR p_place_client IS NOT NULL) THEN
    INSERT INTO public.place (
      client_uuid, name, description, opening_time, closing_time,
...
BEGIN
  ...
  UPDATE public.client SET
    ...
    client_type = (CASE WHEN p_type_choice = 'none' OR p_type_choice = '' THEN 'client' ELSE p_type_choice END)::public.client_type,
...
-- -----------------------------------------------------------------------------
-- delete_client_profile: client + subtype rows (place by client_uuid, like restaurant_client by a_uuid)
-- -----------------------------------------------------------------------------
